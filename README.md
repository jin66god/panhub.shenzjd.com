...
- **多源聚合**：TG 频道 + 6 个第三方搜索插件（影视/网盘资源站），聚合去重、智能排序
...
1. **Telegram 频道**：抓取频道公开网页（`t.me/s/<频道>`），频道清单存于 Turso 加密存储（`channel_config` 表），通过 `CHANNEL_KEY` 解密下发
2. **第三方搜索插件**（6 个注册启用，均为公开资源网站，非 TG 群组）：

| 插件 | 来源站 | 类型 |
|------|--------|------|
| pansearch | pansearch.me | 网盘资源搜索站 |
| melost | melost.cn（影盘社） | 网盘直链搜索 |
| quark4k | quark4k.com | Flarum 论坛（夸克资源） |
| ouge | nxog.eu.org（欧歌影视） | 影视资源站 |
| wanou | nxog.eu.org（万欧影视） | 影视资源站 |
| dyyjv | dyyjv.com（电影云集） | WordPress 影视站 |

> 说明：`server/core/plugins/` 下另有若干未注册/禁用的旧插件文件（pan666、susu 源站被 Cloudflare 拦截；solidtorrents、torrentgalaxy、x1337x、nyaa 为纯磁力站，产品要求不出现磁力链接；yunso、u3c3 已移除），它们不参与搜索，仅为历史保留。
...
├── plugins/                # 6 个注册启用的搜索插件（其余文件为禁用/未注册的旧插件）
│   ├── manager.ts          # 插件注册
│   ├── pluginHealth.ts     # 熔断器
│   └── ...
└── utils/
